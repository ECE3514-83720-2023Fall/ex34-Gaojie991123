...
template <typename TreeItemType, typename FunctionType>
BinaryTree<TreeItemType, FunctionType>
BinaryTree<TreeItemType, FunctionType>::makeBinaryTree(const TreeItemType* array, int low, int high) {
    if (low > high) {
        return BinaryTree<TreeItemType, FunctionType>(); // Return empty tree
    }
    int mid = (low + high) / 2;
